...
Model::Model(ID3D11Device* device, ID3D11DeviceContext* context)
{
	...
	m_world = XMMatrixRotationX(XMConvertToRadians(m_xangle));
	m_world *= XMMatrixRotationY(XMConvertToRadians(m_yangle));
	m_world *= XMMatrixRotationZ(XMConvertToRadians(m_zangle));

	m_world *= XMMatrixScaling(m_scaleX, m_scaleY, m_scaleZ);

	m_world *= XMMatrixTranslation(m_x, m_y, m_z);
...
int Model::LoadObjModel(char* filename)
{
	...
	if (!m_pObject)// check for shader compilation error
...
void Model::draw( XMMATRIX *view, XMMATRIX *projection)
{
	...
	model_cb_values.WorldViewProjection = (m_world)*(*view)*(*projection);
...
void Model::Rotation(float degrees)
{
	m_yangle += degrees;
	m_zangle += degrees;
	m_xangle += degrees;
...
float Model::GetBoundingSphereRadius()
{
	...
	return (scale*m_bounding_sphere_radius);
}
...
{
	D3DX11CreateShaderResourceViewFromFile(m_pD3DDevice, "assets/crate.bmp" , NULL, NULL,&m_pTexture0, NULL);
